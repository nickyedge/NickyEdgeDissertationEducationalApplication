...
local function helpBtnPress(event)
	...
	composer.showOverlay("question8Overlay", {effect = "fade", isModal = true})
end
...
local function updateTimer(event)
    ...
    if counter == 0 then
		...
		composer.removeScene("question8")
		composer.gotoScene("question9", "fade", 500)
    end
end
...
local function onRightAnswer(event)	
	-- go to question9.lua, add a point to score, and remove the timer
	score = score + 5
	scoreDisplay.text = score
	question8 = 1
	composer.removeScene("question8")
...
local function onWrongAnswer1(event)
	-- go to question9.lua, take 5 points from score
	score = score - 5
	scoreDisplay.text = score
	question8 = 0
	composer.removeScene("question8")
...
function scene:create(event)
	...
	question = display.newText([[Question 8: What is a firewall?]], 0, 0, "Helvetica", 16)
	question:setTextColor(0,0,0)
	question.x = display.contentCenterX;question.y = 100
	
	answerOne = display.newText([[A. A wall that is reinforced and 
	cannot catch on fire.]], 0, 0, "Helvetica", 14)
	answerOne:setTextColor(0,0,0)
	answerOne.x = 160;answerOne.y = 255

	answerTwo = display.newText([[B. A program that protects against viruses.]], 0, 0, "Helvetica", 14)
	answerTwo:setTextColor(0,0,0)
	answerTwo.x = 160;answerTwo.y = 330

	answerThree = display.newText([[C. A filter for an internet connection 
	that monitors outgoing and 
	incoming activity.]], 0, 0, "Helvetica", 14)
	...
	rightAnswer.x = 160;rightAnswer.y = 405
	...
	wrongAnswer2.x = 160;wrongAnswer2.y = 255
