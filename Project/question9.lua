...
local function helpBtnPress(event)
	...
	composer.showOverlay("question9Overlay", {effect = "fade", isModal = true})
end
...
local function updateTimer(event)
    ...
    if counter == 0 then
		...
		composer.removeScene("question9")
		composer.gotoScene("question10", "fade", 500)
    end
end
...
local function onRightAnswer(event)	
	-- go to question10.lua, add a point to score, and remove the timer
	score = score + 5
	scoreDisplay.text = score
	question9 = 1
	composer.removeScene("question9")
...
local function onWrongAnswer1(event)
	-- go to question10.lua, take 5 points from score
	score = score - 5
	scoreDisplay.text = score
	question9 = 0
	composer.removeScene("question9")
...
function scene:create(event)
	...
	question = display.newText([[Question 9: A strong password 
	should contain:]], 0, 0, "Helvetica", 16)
	question:setTextColor(0,0,0)
	question.x = display.contentCenterX;question.y = 100
	
	answerOne = display.newText([[A. Both uppercase and lowercase letters.]], 0, 0, "Helvetica", 14)
	answerOne:setTextColor(0,0,0)
	answerOne.x = 160;answerOne.y = 255

	answerTwo = display.newText([[B. A word that is easy to remember, 
	such as the name of a pet.]], 0, 0, "Helvetica", 14)
	answerTwo:setTextColor(0,0,0)
	answerTwo.x = 160;answerTwo.y = 330

	answerThree = display.newText([[C. At least 8 characters, 
	a combination of letters, 
	numbers, and characters.]], 0, 0, "Helvetica", 14)
