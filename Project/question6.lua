...
local function helpBtnPress(event)
	...
	composer.showOverlay("question6Overlay", {effect = "fade", isModal = true})
end
...
local function updateTimer(event)
    ...
    if counter == 0 then
		...
		composer.removeScene("question6")
		composer.gotoScene("question7", "fade", 500)
    end
end
...
local function onRightAnswer(event)	
	-- go to question7.lua, add a point to score, and remove the timer
	score = score + 5
	scoreDisplay.text = score
	question6 = 1
	composer.removeScene("question6")
...
function scene:create(event)
	...
	question = display.newText([[Question 6: Both email attachments and 
	downloaded files can spread malware.]], 0, 0, "Helvetica", 16)
