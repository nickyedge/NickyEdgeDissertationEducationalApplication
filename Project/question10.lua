...
local function helpBtnPress(event)
	...
	composer.showOverlay("question10Overlay", {effect = "fade", isModal = true})
end
...
local function updateTimer(event)
    ...
    if counter == 0 then
		...
		composer.removeScene("question10")
		composer.gotoScene("finalScore", "fade", 500)
    end
end
...
local function onRightAnswer(event)	
	-- go to finalScore.lua, add a point to score, and remove the timer
	score = score + 5
	scoreDisplay.text = score
	question10 = 1
	composer.removeScene("question10")
...
local function onWrongAnswer1(event)
	-- go to finalScore.lua, take 5 points from score
	score = score - 5
	scoreDisplay.text = score
	question10 = 0
	composer.removeScene("question10")
...
function scene:create(event)
	...
	question = display.newText([[Question 10: Social engineering is:]], 0, 0, "Helvetica", 16)
	question:setTextColor(0,0,0)
	question.x = display.contentCenterX;question.y = 100
	
	answerOne = display.newText([[A. A website created for people 
	who share common interests.]], 0, 0, "Helvetica", 14)
	answerOne:setTextColor(0,0,0)
	answerOne.x = 160;answerOne.y = 255

	answerTwo = display.newText([[B. Scams distributed through email 
	such as phishing, pharming 
	and impersonation.]], 0, 0, "Helvetica", 14)
	answerTwo:setTextColor(0,0,0)
	answerTwo.x = 160;answerTwo.y = 330

	answerThree = display.newText([[C. A targeted attack on a 
	personal computer.]], 0, 0, "Helvetica", 14)
